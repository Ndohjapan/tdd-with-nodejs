...
describe('Delete Hoax', () => {
  ...
  it("returns 403 when the user tries to delete another user's hoax", async () => {
  ...
  it('returns 200 ok when a user deletes their hoax', async () => {
    const user = await addUser();
    const hoax = await addHoax(user.id);
  ...
  it('removes the hoax from the database when a user deletes their hoax', async () => {
    const user = await addUser();
    const hoax = await addHoax(user.id);
    const token = await auth({ auth: credentials });
  ...
  it('removes the file attachment from the database when the user deletes hoax ', async () => {
    const user = await addUser();
    const hoax = await addHoax(user.id);
